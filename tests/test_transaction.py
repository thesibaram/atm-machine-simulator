        ...
        if amount > 0:
            if amount <= self.account.get_balance():
                if self.pin_validation():
                    ...
                    self.transaction_history.append(f"Withdrawal: {amount} was successfully withdrawn from your account.")
                else:
                    print("Transaction failed: Invalid PIN.")
            else:
                print("Transaction failed: Insufficient funds for this withdrawal.")
        else:
            print("Transaction failed: The withdrawal amount must be at least 1.")
